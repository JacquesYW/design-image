...
const schema: AnimationSchema = {
  type: 'swing',
  name: '摆动',
  previewImage: 'https://cdn2.weimob.com/saas/saas-fe-sirius-orion-node/production/zh-CN/468/swing.gif',
  defaults: {
    dir: DEFAULT_DIR,
      ...
      init: (layerData, animationSetting) => {
        ...
        if (layerData) {
          dir.split('').forEach((part) => {
            switch (part) {
              case 'l':
                Fn.verify.isNotEmpty(layerData.tr.x) && ((from.x = layerData.tr.x), (to.x = layerData.tr.x - distance));
                break;
              case 'r':
                Fn.verify.isNotEmpty(layerData.tr.x) && ((from.x = layerData.tr.x), (to.x = layerData.tr.x + distance));
                break;
              case 't':
                Fn.verify.isNotEmpty(layerData.tr.y) && ((from.y = layerData.tr.y), (to.y = layerData.tr.y - distance));
                break;
              case 'b':
                Fn.verify.isNotEmpty(layerData.tr.y) && ((from.y = layerData.tr.y), (to.y = layerData.tr.y + distance));
                break;
      ...
      init: (layerData, animationSetting) => {
        ...
        if (layerData) {
          dir.split('').forEach((part) => {
            switch (part) {
              case 'l':
                Fn.verify.isNotEmpty(layerData.tr.x) && ((from.x = layerData.tr.x - distance), (to.x = layerData.tr.x));
                break;
              case 'r':
                Fn.verify.isNotEmpty(layerData.tr.x) && ((from.x = layerData.tr.x + distance), (to.x = layerData.tr.x));
                break;
              case 't':
                Fn.verify.isNotEmpty(layerData.tr.y) && ((from.y = layerData.tr.y - distance), (to.y = layerData.tr.y));
                break;
              case 'b':
                Fn.verify.isNotEmpty(layerData.tr.y) && ((from.y = layerData.tr.y + distance), (to.y = layerData.tr.y));
                break;
